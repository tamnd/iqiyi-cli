// Package cli builds the iqiyi command tree on top of the iqiyi library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: iqiyi.DefaultConfig()}

	root := &cobra.Command{
		Use:   "iqiyi",
		Short: "A command line for iQIYI.",
		Long: `iqiyi reads iQIYI (iqiyi.com) through the public pcw-api.
No API key or authentication required. It returns records as table, JSON,
JSONL, CSV, TSV, or URLs.

iqiyi is an independent tool and is not affiliated with iQIYI.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = iqiyi.NewClient(a.cfg)
	return nil
}
