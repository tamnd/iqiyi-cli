...
describes what users get. Pick a format with `-o`, or let iqiyi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
iqiyi <command> -o table   # aligned columns for reading
iqiyi <command> -o jsonl   # one JSON object per line, for piping
iqiyi <command> -o json    # a single JSON array
iqiyi <command> -o csv     # spreadsheet friendly
iqiyi <command> -o tsv     # tab-separated
iqiyi <command> -o url     # just the URL column
iqiyi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
iqiyi <command> --fields id,title,url
```
...
```bash
iqiyi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
iqiyi <command>            # a table, because this is a terminal
iqiyi <command> | wc -l    # JSONL, because this is a pipe
```
